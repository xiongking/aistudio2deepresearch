...
export class DeepResearchService {
  ...
  private async writeChapter(
    ...
    const prompt = `
      ...
      6. **可视化 (严格执行)**: 必须包含一个 Mermaid.js 图表。
         - **禁止使用 quadrantChart**: 由于渲染不稳定，**严禁**使用四象限图。
         - **推荐使用**: \`graph TD\` (流程图), \`pie\` (饼图), \`sequenceDiagram\` (时序图)。
         - **语法规范**:
           - 仅使用英文ID (如 NodeA)，不要在 ID 中使用中文或特殊字符。
           - 节点文本必须用英文双引号包裹 (如 NodeA["中文文本"])。
           - 不要使用任何复杂的样式类或 callback 定义。
           - 严禁在图表代码块中提及 "mermaid" 字眼，仅用 "下图展示..." 引出代码块。
